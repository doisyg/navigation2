...
| ClearEntireCostmap | Action | Call the ClearEntireCostmap ROS2 service server, which is implemented by Costmap2DROS. This action is used in nav2 Behavior Trees as a recovery behavior. The server address needs to be specified using the `service_name` input port, typically `global_costmap/clear_entirely_global_costmap` or `local_costmap/clear_entirely_local_costmap`. |
| ClearCostmapExceptRegion | Action | Call the ClearCostmapExceptRegion ROS2 service server, which is implemented by Costmap2DROS. This action is used in nav2 Behavior Trees as a recovery behavior. The server address needs to be specified using the `service_name` input port, typically `global_costmap/clear_except_global_costmap` or `local_costmap/clear_except_local_costmap`. |
| ClearCostmapAroundRobot | Action | Call the ClearCostmapAroundRobot ROS2 service server, which is implemented by Costmap2DROS. This action is used in nav2 Behavior Trees as a recovery behavior. The server address needs to be specified using the `service_name` input port, typically `global_costmap/clear_around_global_costmap` or `local_costmap/clear_around_local_costmap`. |
